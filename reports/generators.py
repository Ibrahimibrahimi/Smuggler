...
from scanner.detectors import Finding, Confidence
...
<div class="report-header">
  ...
  <p class="meta">Scan Report · Generated {timestamp} · {total_targets} target(s) scanned</p>
</div>
...
<div class="footer">Generated by HTTP Request Smuggler · For authorized security testing only</div>
</div>
</body>
</html>"""
...
<div class="finding">
  ...
  <div class="finding-body">
    ...
    {'<div class="remediation">🛡️ <strong>Remediation:</strong> ' + f.remediation + '</div>' if f.remediation else ''}
    {refs_html}
  </div>
</div>"""
...
def generate_html(results: List[ScanResult], output_dir: str) -> str:
    out_dir = _ensure_output_dir(output_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
def generate_pdf(results: List[ScanResult], output_dir: str) -> str:
    ...
    html_path = out_dir / f"_tmp_report_{timestamp}.html"
    generate_html(results, output_dir)

    # Find the most recent HTML file
    html_files = sorted(out_dir.glob("smuggler_report_*.html"), reverse=True)
    if not html_files:
        raise FileNotFoundError("HTML report not found for PDF conversion")

    html_src = html_files[0]

    try:
        from weasyprint import HTML
        HTML(filename=str(html_src)).write_pdf(str(pdf_path))
    except ImportError:
        raise RuntimeError(
    ...
    for fmt in formats:
        ...
        try:
            if fmt == "json":
                ...
                generated["html"] = generate_html(results, output_dir)
            elif fmt == "pdf":
